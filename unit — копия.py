...
##    number_of_SKUs = entries_top[1].get()  # единиц товара в грузе       
##    dollars = entries_top[2].get()       # курс $
##    invois_price = entries_top[3].get()  # цена по инвойсу вместе с шопинг $
##    polnaya_summa = entries_top[4].get()  # общая сумма что была переведена за купоенный груз
##    price_posredic = int(polnaya_summa) - int(invois_price) * int(dollars) 
##    sku = entries_bottom[0].get()        # SKU
##    price_per_piece = entries_bottom[1].get()  # цена за шт $
##    schonig = entries_bottom[2].get()    # шопинг общ $
##    delivery = entries_bottom[3].get()   # Доставка мск $
##    tt = entries_bottom[4].get()         # ТТ до дома руб
##    force_majeure = entries_bottom[5].get()
##    force_majeure1 = entries_bottom[6].get()
##    force_majeure2 = entries_bottom[7].get()
##    re = (int(price_per_piece) + int(schonig) / int(number_of_SKUs) + int(delivery) / int(number_of_SKUs)) * int(dollars) + int(tt) / int(number_of_SKUs) + int(price_posredic) / int(number_of_SKUs)
##    re2 = round(re, 1)
##    # Вывод данных в консоль
##    print("Данные:")
##    print(f"№ груза, SKU и дата: {date_and_No}")
##    print(f"Курс доллара на Алиэкспресс: {dollars}")
##    print(f"Количество мест в грузе: {number_of_SKUs}")
##    print(f"SKU: {sku}")
##    print(f"Цена за шт $: {price_per_piece}")
##    print(f"Шопинг общ $: {schonig}")
##    print(f"Доставка мск $: {delivery}")
##    print(f"ТТ до дома руб: {tt}")
##    print(f"Поле 10: {force_majeure}")
##    print(f"Поле 11: {force_majeure1}")
##    print(f"Поле 12: {force_majeure2}")
##    print(f"Вы заплатили посреднику {price_posredic}")
##    print(re2)
